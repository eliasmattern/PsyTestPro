...
def create_schedule_display(schedule, participant_info, teststarter, custom_variables, isHab=False):
    ...
    def button_back():
        ...
        sorted_schedule = sorted([(datetime.strptime(info['datetime'], '%d/%m/%Y %H:%M:%S'), event) for event, info in
        ...
        while running:
            ...
            if edit_button_x + edit_button_width > mouse[0] > edit_button_x and edit_button_y + edit_button_height > \
                ...
                if click[0] == 1:
                    create_schedule_display(schedule, participant_info, teststarter, custom_variables,
                                            isHab)  # Call create_schedule_display() when the button is clicked
            else:
                pygame.draw.rect(screen, light_grey,
                                 (edit_button_x, edit_button_y, edit_button_width, edit_button_height))
            if next_event:
    ...
    def button_quit_teststarter():
        ...
        response = messagebox.askyesno(translate_service.get_translation('confirmExit'),
                                       translate_service.get_translation('confirmExitText'))
    ...
    def button_help():
        ...
        root.geometry('+' + str(posX) + '+' + str(posY))
        # Show a messagebox asking for confirmation
        response = messagebox.askyesno(translate_service.get_translation('help'),
                                       translate_service.get_translation('helpText'))
    ...
    max_row_height = 75
    row_height = screen_height // (len(splitted_schedule[schedule_page]) + 3) if screen_height // (
                len(splitted_schedule[schedule_page]) + 3) < max_row_height else max_row_height  # +1 for the header row
    ...
    left_button = Button(column_start_x + 1.5 * column_width, 760 * height_scale_factor, 40 * width_scale_factor,
                         40 * height_scale_factor, '<', lambda: page_update(splitted_schedule, False), border_radius=90)
    right_button = Button(column_start_x + 2.5 * column_width, 760 * height_scale_factor, 40 * width_scale_factor,
                          40 * height_scale_factor, '>', lambda: page_update(splitted_schedule, True), border_radius=90)
    ...
    while True:
                ...
                for row in range(1, len(splitted_schedule[schedule_page]) + 1):
                    ...
                    if todo_input_box_rect.collidepoint(mouse_pos):
                        ...
                        todo_input_values[row] = state_iterator[
                            splitted_schedule[schedule_page][list(splitted_schedule[schedule_page])[row - 1]]['state']]
                    ...
                    if not isHab:
                        if newdate_input_box_rect.collidepoint(mouse_pos) and not isHab:
                            ...
                            splitted_schedule[schedule_page][list(splitted_schedule[schedule_page])[row - 1]][
                                'datetime'].split(' ')[0]
        ...
        if len(splitted_schedule) > 1:
            page_number_surface = font.render(str(schedule_page + 1) + '/' + str(len(splitted_schedule)), True,
                                              light_grey)
        ...
        text_surface = font.render(' ' + translate_service.get_translation('task'), True,
        ...
        if not isHab:
            text_surface = font.render(' ' + translate_service.get_translation('date'), True,
                                       light_grey)  # Render the text 'Time'
            screen.blit(text_surface,
                        (column_start_x + column_width, cellPadding))  # Blit the text surface to the screen
            text_surface = font.render(' ' + translate_service.get_translation('time'), True,
                                       light_grey)  # Render the text 'Time'
            screen.blit(text_surface,
        ...
        text_surface = font.render(' ' + translate_service.get_translation('skipDoneTodo'), True,
                                   light_grey)  # Render the text 'Skip/newtimedate'
        screen.blit(text_surface, (
        ...
        for task, info in splitted_schedule[schedule_page].items():
            ...
            if row in todo_input_values:
                ...
                todo_input_text_surface = todo_input_box_font.render(
                    translate_service.get_translation(todo_input_value), True, todo_color[todo_input_value.lower()])
                ...
                todo_input_text_surface = todo_input_box_font.render(translate_service.get_translation(state), True,
                                                                     todo_color[splitted_schedule[schedule_page][
                                                                         list(splitted_schedule[schedule_page])[
                                                                             row - 1]]['state'].lower()])
                screen.blit(todo_input_text_surface, todo_input_box_rect.move(5, 5))
            ...
            for i in range(1, max_grid_rows + 2):
                y = i * row_height
                pygame.draw.line(screen, light_grey, (column_start_x, y),
