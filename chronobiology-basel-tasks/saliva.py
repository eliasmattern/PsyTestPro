...
def saliva(subject, experiment, week_number, kss_number):
    ...
    speichelprobe_rendered_line_rect = speichelprobe_rendered_line.get_rect(left=left_side_x,
    ...
    while running:
        ...
        if timer_start_time is None:  # If the timer has not yet been started
            if not timer_started:  # If the timer has not yet been started
                ...
                button_rect = pygame.Rect(button_x, button_y, button_width,
                                          button_height)  # store the button rectangle dimensions in variable button_rect
                ...
                button_text_rect = button_text_rendered.get_rect(
                    center=button_rect.center)  # draw a rectangle around the button text and position the text in the middle of the button
        ...
        if not kss_completed:  # Only render if KSS Fragebogen is not completed
            ...
            submission_text_line1_rect = submission_text_line1_rendered.get_rect(
                center=(submission_text_line1_x, submission_text_line1_y))
            ...
            for i, line in enumerate(question_lines):
                ...
                question_text_rect = question_text_rendered.get_rect(left=question_x,
                                                                     top=question_y + i * 30 * height_scale_factor)
                ...
                for j, option in enumerate(options):
                    ...
                    option_text_rect = option_text_rendered.get_rect(left=option_x,
                                                                     top=option_y + j * 30 * height_scale_factor)
                    ...
                    tick_box_rect = pygame.Rect(option_x - 30 * width_scale_factor,
                                                option_y + j * 30 * height_scale_factor, 20 * width_scale_factor,
                    ...
                    if selected_option == option:
                        ...
                        tick_mark_points = [
                            (option_x - 25 * width_scale_factor,
                             option_y + j * 30 * height_scale_factor + 10 * height_scale_factor),
                            (option_x - 20 * width_scale_factor,
                             option_y + j * 30 * height_scale_factor + 15 * height_scale_factor),
                            (option_x - 15 * width_scale_factor,
                             option_y + j * 30 * height_scale_factor + 5 * height_scale_factor)
                        ]
        ...
        submission_text_line2_rect = submission_text_line2_rendered.get_rect(
            center=(submission_text_line2_x, submission_text_line2_y))
            ...
            if event.type == pygame.QUIT:  # check if the event is of type pygame.QUIT, which is triggered when the user attempts to close the game window (e.g., by clicking the close button)
                ...
                if event.key == pygame.K_ESCAPE:  # check if the event.key corresponds to the pygame.K_ESCAPE constant, which represents the Escape key on the keyboard
                    ...
                    if selected_option is not None:  # check if an option has been chosen to answer the KSS
                        if not kss_completed:  # if the KSS Fragebogen has not yet been completed
                            kss_result = selected_option.split('.')[
                                0].strip()  # Save the number (1st position) of the selected option to a variable kss_result
                            ...
                            with open(output_filename, 'a', newline='') as csvfile:
                                ...
                                if not csv_exists:
                                    writer.writerow(
                                        ["Day", "Month", "Year", "Hours", "Minutes", "Seconds", 'Subject ID', 'Block',
                                         'KSS Number', 'müdigkeit'])
                                # Write the output to the csv
                                writer.writerow(
                                    [day, month, year, hour, minute, second, subject, week_number_str, kss_number,
                                     kss_result])
                ...
                if event.button == 1:  # If the left mouse button clicked
                    ...
                    if button_rect.collidepoint(
                            mouse_pos):  # If the cursor position has collided with the start timer button
                        ...
                        for i, option in enumerate(options):
                            ...
                            tick_box_rect = pygame.Rect(option_x - 30 * width_scale_factor,
                                                        option_y + i * 30 * height_scale_factor,
                                                        20 * width_scale_factor, 20 * height_scale_factor)
...
if len(sys.argv) >= 5:
    subject, experiment, week_number, kss_number = sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4]
    saliva(subject, experiment, week_number, kss_number)
else:
