...
#########################################################################################
def practice_nback(subject, experiment, week, study_night, current_block, num_back, num_back_addition,
                   num_back_performance_addition):
    ...
    screen = pygame.display.set_mode((0, 0), pygame.FULLSCREEN)  # Use pygame.FULLSCREEN flag
    ...
    # Load the images
    correct_image = pygame.image.load("./chronobiology-basel-tasks/Correct.png")
    incorrect_image = pygame.image.load("./chronobiology-basel-tasks/Incorrect.png")
    ...
    with open(output_filename, 'a', newline='') as csvfile:
        ...
        writer.writerow(
            ['Subject ID', "Day", "Month", "Year", "Hours", "Minutes", "Seconds", 'Num-Back', 'Block', 'Current Trial',
             'Current Letter', 'Back Letter', 'Response Time', 'User Input', 'Result'])
        ...
        while True:
            ...
            for index, text in enumerate(first_screen_intro_texts):
                ...
                intro_text_rect = intro_text_surface.get_rect(
                    center=(screen_width // 2, (screen_height // 2 - 200) + (index * line_height)))
            ...
            while running:
                ...
                if current_trial < num_trials:
                    ...
                    if pygame.time.get_ticks() < start_time + blank_duration:
                        ...
                        if current_trial >= num_back:
                            ...
                            writer.writerow([subject, day, month, year, hour, minute, second, num_back, current_block,
                                             current_trial, current_letter, back_letter, response_time, user_input,
                                             result])

                            # Print variables
                            print(
                                f"current block: {current_block} |no of trials: {num_trials} |current trial: {current_trial} | current letter: {current_letter} | back letter: {back_letter} | user input: {user_input} | result: {result} ")
                ...
                if current_trial == 10:
                    ...
                    practice_feedback_rect = practice_feedback_surface.get_rect(
...
def real_nback(subject, experiment, week, study_night, current_block, num_back, num_back_addition,
               num_back_performance_addition):
    # Always open the pygame window at front of all windows open on screen
    os.environ['SDL_VIDEO_WINDOW_POS'] = '0,0'  # Set window position to top-left corner
            ...
            while True:
                ...
                while running:
                    ...
                    if current_trial < num_trials:
                        ...
                        # Write the data to the CSV file
                        writer.writerow(
                            [subject, day, month, year, hour, minute, second, num_back, current_block, current_trial,
                             current_letter, back_letter, response_time, user_input, result])
                ...
                if current_block == total_num_blocks:
                    ...
                    block_feedback_rect = block_feedback_surface.get_rect(
                        center=(screen_width // 2, screen_height // 2 - 100))
                    ...
                    current_block += 1
                    # current_trial = 0
                    practice_nback(subject, experiment, week, study_night, current_block, num_back, num_back_addition,
                                   num_back_performance_addition)
                    # Clear the screen for the next block
                    # screen.fill(black)
                    # pygame.display.flip()
...
def start_real_nback(subject, experiment, week, study_night):
    ...
    num_back_addition = None
    num_back_performance_addition = 0
    result_practice = practice_nback(subject, experiment, week, study_night, current_block, num_back, num_back_addition,
                                     num_back_performance_addition)
    result_real = real_nback(subject, experiment, week, study_night, current_block, num_back, num_back_addition,
                             num_back_performance_addition)
...
if len(sys.argv) >= 5:
    subject, experiment, week, study_night = sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4]
    start_real_nback(subject, experiment, week, study_night)
else:
