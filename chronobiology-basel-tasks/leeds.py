...
def leeds(subject, block, experiment):
	...
	keyboard_filename = os.path.join("./chronobiology-basel-tasks", "LeedsKeyboard.csv")
	slider_filename = os.path.join("./chronobiology-basel-tasks", "LeedsSlider.csv")
	...
	slider = Slider(window, percent_screen_width * 20, round(screen_height / 2), percent_screen_width * 60, 40, min=0,
			...
			for event in pygame.event.get():
				if event.type == pygame.KEYDOWN:
					...
					if button_rect.collidepoint(mouse_pos):
						...
						running = False
						# update screen
						slider = Slider(window, percent_screen_width * 20, round(screen_height / 2),
...
if len(sys.argv) >= 3:
	subject, block, experiment = sys.argv[1], sys.argv[2], sys.argv[3]
	leeds(subject, block, experiment)
else:
