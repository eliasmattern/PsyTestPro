...
def GoNoGo_Real(subject, experiment,block,number):
    ...
    output_file = os.path.join(title_dir,
    ...
    def write_data(subject, experiment, block, number, data):
        date = datetime.datetime.now()
        filename = os.path.join(title_dir, f"Gonogo_{subject}_{experiment}_{block}_{number}_{date.day}_{date.month}_{date.year}.csv")
